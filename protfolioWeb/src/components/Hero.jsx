...
export function Hero() {
  ...
  return <section id="home" className="hero container">
    ...
    <motion.div className="hero-copy" initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: .7 }}>
      ...
      <div className="hero-actions"><a className="button primary" href={`${base}resume.pdf`} download>Download Resume <ArrowDownToLine size={17}/></a><a className="button ghost" href="#contact">Contact me <ArrowUpRight size={17}/></a></div>
    ...
    <motion.div className="hero-visual" initial={{ opacity: 0, scale: .9 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: .75, delay: .1 }}>
      <div className="orbit orbit-a"/><div className="orbit orbit-b"/><div className="portrait-ring"><img src={`${base}images/mahadev-portrait-source.png`} alt="Mahadev Guchhait, QA Automation Engineer"/></div>
